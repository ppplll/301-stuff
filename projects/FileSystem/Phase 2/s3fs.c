...
void cpystat(s3dirent_t *dir, struct stat *statbuf, int i){
    (*statbuf).st_mode = dir[0].st_mode;
    (*statbuf).st_nlink = dir[0].st_nlink;
    (*statbuf).st_uid = dir[0].st_uid;
    (*statbuf).st_gid = dir[0].st_gid;
    (*statbuf).st_size = dir[0].st_size;
    (*statbuf).st_atime = dir[0].atime;
    (*statbuf).st_mtime = dir[0].mtime;
    (*statbuf).st_ctime = dir[0].ctime; 
    return;  //i feel like im back in detention: i will not... i will not...
 }
/* 
...
int fs_getattr(const char *path, struct stat *statbuf) {
    ...
    if (type == 2){
        ...
        while (!strcmp(dir[i].name,parent) && (i < dirsize)){
            i++;
        }
        if (i != dirsize){
